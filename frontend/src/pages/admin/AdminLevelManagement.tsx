...
const AdminLevelManagement: React.FC = () => {
  ...
  const { levels, loading, error, refetch, templateName, setTemplateName } = useLevelData('A');
  ...
  const handleCloneTemplate = async (from: string, to: string) => {
    if (!token) return;
    try {
      setFetchingTemplates(true);
      const response = await apiFetch('/level/clone', {
        ...
        body: JSON.stringify({ fromTemplate: from, toTemplate: to })
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleDownloadLevel = async (levelNumber: number) => {
    try {
      const response = await apiFetch(`/level/${levelNumber}?templateName=${templateName}`);
      const data = await response.json();
          ...
          <div className="pt-20 pb-20">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-end mb-8">
              <div className="flex-1 max-w-xs">
                <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 block">Active Template</label>
                <select
                  value={templateName}
                  onChange={(e) => setTemplateName(e.target.value)}
                  className="w-full bg-background border border-border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500/50 outline-none transition-all"
                >
                  {templates.map(t => (
                    <option key={t} value={t}>Template {t}</option>
                  ))}
                  {!templates.includes('A') && <option value="A">Template A</option>}
                </select>
              </div>
              <div className="flex gap-2">
                <Button
                  onClick={() => {
                    const name = prompt('Enter name for the NEW template:');
                    if (name && name.trim()) {
                      const shouldClone = confirm(`Do you want to clone all levels from "${templateName}" into "${name.trim()}"?`);
                      if (shouldClone) {
                        handleCloneTemplate(templateName || 'A', name.trim());
                      } else {
                        setTemplateName(name.trim());
                        if (!templates.includes(name.trim())) {
                          setTemplates([...templates, name.trim()]);
                        }
                      }
                    }
                  }}
                ...
                >
                  <RefreshCw className={fetchingTemplates ? "w-4 h-4 animate-spin" : "w-4 h-4"} />
                  Create Template
                </Button>

                <Button
                  onClick={() => {
                    const name = prompt(`Clone "${templateName}" to a new name:`);
                    if (name && name.trim()) {
                      handleCloneTemplate(templateName || 'A', name.trim());
                    }
                  }}
            ...
            <MagicBadge title={`Editing Template: ${templateName}`} className="mb-6" />
